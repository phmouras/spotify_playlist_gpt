...
def validate_corpus(vf, indx_part, vocab_size, expected_rows):
    """
    Valida corpus e index.
    """

    if len(vf) == 0:
        raise ValueError("O corpus está vazio.")

    if vf.dtype != np.int32:
        raise TypeError(
            f"Esperado vf em np.int32, recebido {vf.dtype}."
        )

    if vf.min() < 0:
        raise ValueError("O corpus possui tokens negativos.")

    if vf.max() >= vocab_size:
        raise ValueError(
            f"Token máximo {vf.max()} >= vocab_size {vocab_size}."
        )

    num_playlists = len(indx_part)

    num_delimiters = np.count_nonzero(vf == 0)

    if num_delimiters != num_playlists:
        raise ValueError(
            f"Foram encontrados {num_delimiters} delimitadores "
            f"para {num_playlists} playlists."
        )

    if int(indx_part["num_tracks"].sum()) != expected_rows:
        raise ValueError(
            "A soma de num_tracks não corresponde ao número "
            "de músicas carregadas."
        )

    if not (indx_part["inicio"] < indx_part["fim"]).all():
        raise ValueError(
            "Existem playlists com índices inválidos."
        )